...
func TestSysFSRaplPowerMeter(t *testing.T) {
	...
	for _, zone := range zones {
		...
		assert.GreaterOrEqual(t, zone.MaxEnergy(), 1000.0*device.Joule, "Max energy should not be negative")
		...
		assert.GreaterOrEqual(t, energy, 1000.0*device.Joule, "Energy should not be negative")
	}
}
